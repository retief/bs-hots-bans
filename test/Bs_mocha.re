...
external fail : 'a => 'a => Js.undefined string => string => unit =
  "fail" [@@bs.val] [@@bs.module "assert"];

external dump : array 'a => unit = "console.log" [@@bs.val] [@@bs.splice];
 ...
 */
let from_suites name (suite: list (string, 'a => unit)) =>
  describe name ((fun () => List.iter (fun (name, code) => it name code) suite) [@bs]);
...
let from_pair_suites name (suites: pair_suites) =>
  describe name @@ (fun () =>
    suites |> List.iter @@ fun (name, code) =>
      it name @@ fun _ => switch (code ()) {
        | Eq a b [@implicit_arity] => assert_equal a b
        | Neq a b [@implicit_arity] => assert_notequal a b
        | Ok a => assert_ok a
        | Approx a b [@implicit_arity] =>
          if (not (close_enough a b)) {assert_equal a b} /* assert_equal gives better ouput */
        | ApproxThreshold t a b [@implicit_arity] =>
          if (not (close_enough threshold::t a b)) {assert_equal a b} /* assert_equal gives better ouput */
